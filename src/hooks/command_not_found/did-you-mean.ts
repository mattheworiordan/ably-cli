...
const findClosestCommand = (
  ...
  const distances = possibilities.map((id) => ({
    distance: levenshteinDistance(target, id, { useCollator: true }),
    id,
  }));
...
const hook: Hook<"command_not_found"> = async function (opts) {
  ...
  if (!suggestion) {
    this.error(
      `Command ${chalk.yellow(originalCmd)} not found. Run ${chalk.cyan.bold(binHelp)} for a list of available commands.`,
      { exit: 127 },
    );
    return;
  }
  ...
  this.warn(
    `${chalk.yellow(originalCmd)} is not an ${opts.config.bin} command.`,
  );
