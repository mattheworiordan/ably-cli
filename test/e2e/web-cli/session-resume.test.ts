...
async function waitForCliReady(page: import('playwright/test').Page, locatorStr = '.xterm', timeout = 60000) {
  await expect(page.locator(locatorStr)).toContainText(BANNER_TEXT, { timeout });
}

test.describe('Web CLI Session Resumption', () => {
  ...
  test('resumes session after abrupt WebSocket disconnect', async () => {
    ...
    await waitForCliReady(page, '.xterm', 30000);
    ...
    await waitForCliReady(page, '.xterm', 60000);
    ...
    // Press ArrowUp to recall last command (bash history) and verify it appears
    await page.keyboard.press('ArrowUp');
    // Small delay to allow terminal to render recalled command
    await page.waitForTimeout(300);
    await expect(terminal).toContainText('ably --version', { timeout: 5000 });
