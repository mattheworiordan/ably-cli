...
export default class SpacesLocksIndex extends Command {
  ...
  async run(): Promise<void> {
    ...
    this.log(
      "  ably spaces locks subscribe   - Subscribe to lock changes in a space",
    );
    this.log(
      "  ably spaces locks get         - Get information about a specific lock",
    );
    this.log(
      "  ably spaces locks get-all     - Get all current locks in a space",
    );
    this.log("");
    this.log(
      "Run `ably spaces locks COMMAND --help` for more information on a command.",
    );
  }
}
