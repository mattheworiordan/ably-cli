...
export default class Config extends AblyBaseCommand {
  ...
  static override examples = ["<%= config.bin %> <%= command.id %> edit"];
  ...
  static override flags = {
    ...
    editor: Flags.string({
      char: "e",
      description:
        "Text editor to use (defaults to $EDITOR environment variable)",
    }),
  };
  ...
  public async run(): Promise<void> {
    ...
    if (!editor) {
      this.error(
        "No text editor found. Please set one with the --editor flag or set the $EDITOR environment variable.",
      );
      return;
    }
