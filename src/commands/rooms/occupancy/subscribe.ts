    ...
    try {
      ...
      try {
         ...
         this.displayOccupancyMetrics(initialOccupancy, roomId, flags, true);
          ...
          this.logCliEvent(flags, 'occupancy', 'getInitialError', errorMsg, { error: errorMsg });
           if (!this.shouldOutputJson(flags)) {
      ...
      this.unsubscribeOccupancyFn = room.occupancy.subscribe((occupancyMetrics: OccupancyEvent) => {
