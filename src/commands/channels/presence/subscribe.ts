  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      this.logCliEvent(
        ...
        `Fetching initial presence members for channel ${channelName}`,
      );
      if (!this.shouldOutputJson(flags)) {
        this.log(
          `Fetching current presence members for channel ${chalk.cyan(channelName)}...`,
        );
      }
      ...
      this.logCliEvent(
        ...
        "presence",
        "subscribingToEvents",
        "Subscribing to subsequent presence events",
      );
      if (!this.shouldOutputJson(flags)) {
      ...
      channel.presence.subscribe("enter", (presenceMessage) => {
        ...
        this.logCliEvent(
          ...
          "presence",
          "memberEntered",
          `Member entered presence: ${presenceMessage.clientId}`,
          { channel: channelName, member: memberData, timestamp },
        );
        ...
        if (this.shouldOutputJson(flags)) {
          ...
          this.log(
            `[${chalk.dim(timestamp)}] ${chalk.green("✓")} ${chalk.blue(presenceMessage.clientId || "Unknown")} entered presence`,
          );

          if (
            presenceMessage.data &&
            Object.keys(presenceMessage.data).length > 0
          ) {
            this.log(
              `  Data: ${this.formatJsonOutput(presenceMessage.data, flags)}`,
            );
          }
        }
      });
      ...
      channel.presence.subscribe("leave", (presenceMessage) => {
        ...
        this.logCliEvent(
          ...
          "presence",
          "memberLeft",
          `Member left presence: ${presenceMessage.clientId}`,
          { channel: channelName, member: memberData, timestamp },
        );
        ...
        if (this.shouldOutputJson(flags)) {
          ...
          this.log(
            `[${chalk.dim(timestamp)}] ${chalk.red("✗")} ${chalk.blue(presenceMessage.clientId || "Unknown")} left presence`,
          );
        }
      });
      ...
      channel.presence.subscribe("update", (presenceMessage) => {
        ...
        this.logCliEvent(
          ...
          "presence",
          "memberUpdated",
          `Member updated presence: ${presenceMessage.clientId}`,
          { channel: channelName, member: memberData, timestamp },
        );
        ...
        if (this.shouldOutputJson(flags)) {
          ...
          this.log(
            `[${chalk.dim(timestamp)}] ${chalk.yellow("⟲")} ${chalk.blue(presenceMessage.clientId || "Unknown")} updated presence data:`,
          );
      ...
      this.logCliEvent(
        ...
        "presence",
        "listening",
        "Now listening for real-time presence events",
      );
          ...
          try {
            try {
              ...
              this.logCliEvent(
                ...
                "presence",
                "unsubscribed",
                "Successfully unsubscribed from presence events",
              );
              if (this.shouldOutputJson(flags)) {
                this.log(
                  this.formatJsonOutput(
              ...
              this.logCliEvent(
                ...
                "presence",
                "unsubscribeError",
                `Error unsubscribing from presence: ${errorMsg}`,
            ...
            if (client) {
              ...
              if (this.shouldOutputJson(flags)) {
                this.log(
                  this.formatJsonOutput(
                    { channel: channelName, status: "closed", success: true },
                    flags,
                  ),
                );
              } else {
      ...
      this.logCliEvent(
        ...
        "presence",
        "fatalError",
        `Error subscribing to presence: ${errorMsg}`,
