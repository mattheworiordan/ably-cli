...
export default class HelpCommand extends Command {
  ...
  async run(): Promise<void> {
    ...
    this.log("");
    this.log(
      "  ably help ask       - Ask a question to the Ably AI agent for help",
    );
    ...
    this.log(
      chalk.yellow(
        "Did you mean ably --help? Here's a list of all commands available for ably:",
      ),
    );
    this.log("");
  ...
  private async displayAllCommands(): Promise<void> {
    ...
    for (const c of this.config.commands) {
      try {
        ...
        if (
          (cmd && "isAlias" in cmd && cmd.isAlias) ||
