...
export default class Auth extends AblyBaseCommand {
  ...
  async run(): Promise<void> {
    ...
    this.log('  ably auth keys switch        - Switch to a key for all client requests')
    this.log('  ably auth issue-jwt-token    - Create an Ably JWT token with capabilities')
    this.log('  ably auth issue-ably-token   - Create an Ably Token with capabilities')
