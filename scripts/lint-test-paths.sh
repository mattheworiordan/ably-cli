...
if [ -n "$TEST_PATTERN" ] && [ "$TEST_PATTERN" != "test/**/*.test.ts" ]; then
  ...
  if echo "$TEST_PATTERN" | grep -q "test.*\.ts"; then
    ...
    if echo "$TEST_PATTERN" | grep -q "test/unit/"; then
    ...
    elif echo "$TEST_PATTERN" | grep -q "test/integration/" || echo "$TEST_PATTERN" | grep -q "test/e2e/"; then
