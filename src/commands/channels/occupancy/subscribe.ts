    ...
    try {
      ...
      channel.subscribe('[meta]occupancy', (message: Ably.Message) => {
        ...
        if (!occupancyMetrics) {
          ...
          this.logCliEvent(flags, 'subscribe', 'metricsUnavailable', errorMsg, { channel: channelName, timestamp });
          if (this.shouldOutputJson(flags)) {
            this.log(this.formatJsonOutput({ channel: channelName, error: errorMsg, success: false, timestamp }, flags))
          } else {
      ...
      await new Promise<void>((resolve) => {
        const cleanup = () => {
          ...
          try {
             ...
             this.logCliEvent(flags, 'subscribe', 'unsubscribedOccupancy', `Unsubscribed from [meta]occupancy on ${channelName}`, { channel: channelName });
               ...
               this.logCliEvent(flags, 'subscribe', 'unsubscribeError', `Error unsubscribing from ${channelName}: ${error instanceof Error ? error.message : String(error)}`, { channel: channelName, error: error instanceof Error ? error.message : String(error) });
          }
        ...
        this.logCliEvent(flags, 'subscribe', 'fatalError', `Error during occupancy subscription: ${errorMsg}`, { channel: channelName, error: errorMsg });
        if (this.shouldOutputJson(flags)) {
