  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      this.logCliEvent(
        ...
        `Subscribing to messages in room ${roomId}`,
      );
      ...
      this.logCliEvent(
        ...
        `Successfully subscribed to messages in room ${roomId}`,
      );
      ...
      this.logCliEvent(
        ...
        "room",
        "subscribingToStatus",
        `Subscribing to status changes for room ${roomId}`,
      );
      this.unsubscribeStatusFn = room.onStatusChange(
        (statusChange: unknown) => {
          ...
          this.logCliEvent(
            ...
            { reason: change.reason, roomId: roomId },
          );
          if (change.current === "attached") {
            if (!this.shouldSuppressOutput(flags)) {
              if (this.shouldOutputJson(flags)) {
                ...
                this.log(
                  `${chalk.dim("Listening for messages. Press Ctrl+C to exit.")}`,
                );
              }
            }
      ...
      this.logCliEvent(
        ...
        "room",
        "subscribedToStatus",
        `Successfully subscribed to status changes for room ${roomId}`,
      );
      ...
      await new Promise<void>((resolve) => {
        const cleanup = async () => {
          ...
          const forceExitTimeout = setTimeout(() => {
            ...
            this.logCliEvent(flags, "subscribe", "forceExit", errorMsg, {
              roomId,
            });
            if (!this.shouldOutputJson(flags)) {
      ...
      if (this.shouldOutputJson(flags)) {
        this.log(
          this.formatJsonOutput(
            { error: errorMsg, roomId: roomId, success: false },
            flags,
          ),
        );
      } else {
