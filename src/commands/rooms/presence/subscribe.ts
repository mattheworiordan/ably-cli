  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      this.logCliEvent(
        ...
        `Fetching initial presence members for room ${roomId}`,
      );
      if (!this.shouldOutputJson(flags)) {
        this.log(
          `Fetching current presence members for room ${chalk.cyan(roomId)}...`,
        );
      }
      ...
      if (this.shouldOutputJson(flags)) {
        this.log(
          this.formatJsonOutput(
            { members: initialMembers, roomId, success: true },
            flags,
          ),
        );
        ...
        for (const member of members) {
          ...
          if (member.data && Object.keys(member.data).length > 0) {
            this.log(
              `  ${chalk.dim("Data:")} ${this.formatJsonOutput({ data: member.data }, flags)}`,
            );
          }
      ...
      this.presenceSubscription = room.presence.subscribe(
        ...
        (member) => {
          ...
          this.logCliEvent(
            ...
            "presence",
            action,
            `Presence event '${action}' received`,
            eventData,
          );
          ...
          if (this.shouldOutputJson(flags)) {
            ...
            this.log(
              `[${timestamp}] ${actionColor(actionSymbol)} ${chalk.blue(member.clientId || "Unknown")} ${actionColor(action)}`,
            );
            ...
            if (this.unsubscribeStatusFn) {
              try {
                ...
                this.logCliEvent(
                  ...
                  "room",
                  "unsubscribedStatus",
                  "Successfully unsubscribed from status events",
                );
              } catch (error) {
                const errorMsg =
                  error instanceof Error ? error.message : String(error);
      ...
      this.logCliEvent(flags, "presence", "fatalError", `Error: ${errorMsg}`, {
        error: errorMsg,
        roomId: args.roomId,
      });
