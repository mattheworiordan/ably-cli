...
async function _handleAuth(
  ...
  try {
    ...
    const timeoutId = setTimeout(
      () => cleanupAndTerminate("Session timed out."),
      SESSION_TIMEOUT_MS,
    );
        ...
        if (processingBuffer.length >= 8 + payloadSize) {
          ...
          if (streamType === 1) {
            try {
              ...
              logError(
                `[${logSessionId}] WS stream write error after demux: ${error instanceof Error ? error.message : String(error)}`,
              );
    ...
    websocketStream.on("data", (chunk: Buffer | string) => {
      ...
      try {
        ...
        if (isResizeMessage) {
          ...
          if (cols > 0 && rows > 0) {
            log(
              `[${currentSessionId}] Received resize request: ${cols}x${rows}`,
            );
            ...
            if (currentSession && currentSession.execInstance) {
              ...
              currentSession.execInstance
                .resize({ h: rows, w: cols })
                .catch((resizeError) => {
                  logError(
                    `[${currentSessionId}] Failed to resize PTY on request: ${resizeError}`,
                  );
                });
            } else {
              logError(
                `[${currentSessionId}] Could not find session or execInstance to resize PTY.`,
              );
            }
          } else {
            logError(
              `[${currentSessionId}] Received invalid dimensions in resize request: ${cols}x${rows}`,
            );
          }
        } else {
          ...
          if (
            currentSession &&
            currentSession.stdinStream &&
            !currentSession.stdinStream.destroyed
          ) {
            ...
            log(
              `[${currentSessionId}] Could not forward non-resize data, stream unavailable.`,
            );
          }
        }
      } catch (error) {
        ...
        logError(
          `[${currentSessionId}] Error processing incoming websocket data: ${error}`,
        );
    ...
    logError(`Error in handleAuth: ${error instanceof Error ? error.message : String(error)}`);
