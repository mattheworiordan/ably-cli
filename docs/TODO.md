...
- [ ] [feat/terminal-server-improvements] When an API key or Access Token fails with a 40x error, it should show the error message without a stack trace, and should tell the user to reauth appropriately i.e. using `ably login` for access tokens, or `ably auth keys switch` for API keys. Here is the error message from an access token that failed:
  ...
  ```
- [ ] [feat/terminal-server-improvements] Web example will automatically authenticate from the .env file in the example. However, there is no option to change the authentication afterwards in the web example. There should be a small button/link to change the auth, which takes you to the form which shows you the current API key and access token, that can be changed and logged back in.
- [ ] [feat/terminal-server-improvements] Web example index page should provide two options to view the web CLI example. 1) What is the current version, which is just a full screen terminal, 2) A drawer version that sits at the bottom of the page with a small icon, that when clicked opens up a drawer that connects and loads the CLI in the drawer. The drawer by default will take up 50% of the vertical space, but can be expanded or contracted with the user dragging the top of the drawer up or down.
- [ ] [feat/terminal-server-improvements] Web CLI component should the user that it is connecting to the terminal server in the terminal interface with a visual indication (in ASCII) that it's connecting. Once the terminal loads, the terminal interface is cleared and the standard terminal is shown.
...
- [ ] [feat/terminal-server-improvements] The terminal server will support basic connection re-establishment if the connection is abruptly disconnected. The web client should be issued with a session ID from the server when the connection is established, and upon reconnecting, it should pass in that session ID to attempt to resume the session. If the session does not exist (perhaps due to being disconnected for a long time), the user should simply be given a new session. However, if the terminal server has an existing connection that matches that session ID, or has a recently (up to 60 seconds) terminated connection, the terminal server should send the terminal state to the web client from the existing connection thus allowing the connection to continue.
- [ ] [feat/terminal-server-improvements] The web CLI React component will automatically handle resuming connections that are terminated abruptly with the session ID. However, if the page is reloaded, the web CLI component cannot assume it should resume the previous session with an explicit parameter provided to the React component indicating it should follow that behaviour. When configured to resume on reload, the React component should store the session ID in the tab's local storage on unload, and then resume the connection if the session ID exists and then clear the session ID in the local storage after. Additionally, the session ID should be available as a property of the React component.
