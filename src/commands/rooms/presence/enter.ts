  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (flags["profile-data"]) {
        try {
          ...
          this.logCliEvent(
            flags,
            "presence",
            "profileDataParseError",
            errorMsg,
            { error: errorMsg, roomId },
          );
      ...
      if (flags["show-others"]) {
        ...
        const { off: unsubscribeStatus } = room.onStatusChange(
          (statusChange: RoomStatusChange) => {
            ...
            if (
              ...
              this.log(
                `${chalk.green("Successfully connected to room:")} ${chalk.cyan(roomId)}`,
              );
            }
          },
        );
        ...
        const { unsubscribe: unsubscribePresence } = room.presence.subscribe(
          (event) => {
            ...
            if (event.clientId !== this.chatClient?.clientId) {
              ...
              this.logCliEvent(
                ...
                "presence",
                event.action,
                `Presence event '${event.action}' received`,
                eventData,
              );
              ...
              if (this.shouldOutputJson(flags)) {
                ...
                switch (event.action) {
                  case "enter": {
                    this.log(
                      `${chalk.green("✓")} ${chalk.blue(event.clientId || "Unknown")} entered room`,
                    );
                  ...
                  case "leave": {
                    this.log(
                      `${chalk.red("✗")} ${chalk.blue(event.clientId || "Unknown")} left room`,
                    );
                  ...
                  case "update": {
                    this.log(
                      `${chalk.yellow("⟲")} ${chalk.blue(event.clientId || "Unknown")} updated presence data:`,
                    );
                    this.log(
                      `  ${chalk.dim("Data:")} ${this.formatJsonOutput(event.data as Record<string, unknown>, flags)}`,
                    );
      ...
      this.logCliEvent(
        ...
        "presence",
        "entered",
        "Successfully entered presence",
        { profileData: this.profileData },
      );

      if (this.shouldOutputJson(flags)) {
        this.log(
          this.formatJsonOutput({ success: true, ...enterEventData }, flags),
        );
      } else {
        this.log(
          `${chalk.green("✓")} Entered room ${chalk.cyan(roomId)} as ${chalk.blue(this.chatClient?.clientId || "Unknown")}`,
        );
      }
      ...
      if (flags["show-others"]) {
        ...
        this.logCliEvent(
          flags,
          "presence",
          "gettingInitialMembers",
          "Fetching initial presence members",
        );
        const members = await room.presence.get();
        const initialMembers = members
          .filter((member) => member.clientId !== this.chatClient?.clientId)
          .map((member) => ({
        ...
        this.logCliEvent(
          flags,
          "presence",
          "initialMembersFetched",
          `Fetched ${members.length} total members (${initialMembers.length} others)`,
          {
            count: members.length,
            members: initialMembers,
            othersCount: initialMembers.length,
          },
        );
            ...
            for (const member of members) {
              if (member.clientId !== this.chatClient?.clientId) {
                ...
                if (member.data && Object.keys(member.data).length > 0) {
                  this.log(
                    `  ${chalk.dim("Data:")} ${this.formatJsonOutput(member.data as Record<string, unknown>, flags)}`,
                  );
                }
              }
            }
        ...
        this.logCliEvent(
          ...
          "presence",
          "present",
          "Staying present in the room until terminated",
        );
        if (!this.shouldOutputJson(flags)) {
          this.log(
      ...
      this.logCliEvent(
        ...
        "presence",
        "listening",
        "Actively present and listening for events",
      );
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const cleanup = async () => {
          ...
          const forceExitTimeout = setTimeout(() => {
            ...
            this.logCliEvent(flags, "presence", "forceExit", errorMsg, {
              roomId,
            });
            if (this.shouldOutputJson(flags)) {
              this.log(
          ...
          try {
            ...
            try {
              ...
              this.logCliEvent(
                ...
                "presence",
                "left",
                "Successfully left room presence",
              );
              if (this.shouldOutputJson(flags)) {
                this.log(
                  this.formatJsonOutput(
                    { action: "leave", roomId, success: true },
                    flags,
                  ),
                );
              } else {
            ...
            try {
              ...
              if (this.shouldOutputJson(flags)) {
                this.log(
                  this.formatJsonOutput(
                    { action: "release", roomId, success: true },
                    flags,
                  ),
                );
              } else {
      ...
      this.logCliEvent(
        ...
        "presence",
        "fatalError",
        `Error entering room presence: ${errorMsg}`,
